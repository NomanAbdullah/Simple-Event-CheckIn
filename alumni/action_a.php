<?php  
	...
	if(isset($_POST['status'])){
		$RegNo = $_POST['RegNo'];
		$stmt=$conn->prepare("UPDATE tbl_alumni SET CheckIn='Yes' WHERE RegNo=?");
	...
	if (isset($_POST['query'])) {
		$search=$_POST['query'];
		$stmt=$conn->prepare("SELECT * FROM tbl_alumni WHERE RegNo=? OR Contact=?");
		...
		$output ="<thead>
				...
				while($row=$result->fetch_assoc()) {
					...
					<tr>
						...
						<td>".$row['Batch']."</td>
						<td>"
							."<a href=setAlumniStatus.php?id=".$RegNo.">";
										if($Status == "Yes"){ 
